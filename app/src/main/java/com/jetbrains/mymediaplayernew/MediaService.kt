...
class MediaService : Service(), MediaPlayerCallback {
    ...
    companion object {
        const val ACTION_CREATE = "com.jetbrains.mymediaplayernew.mediaservice.create"
        const val ACTION_DESTROY = "com.jetbrains.mymediaplayernew.mediaservice.destroy"
        const val TAG = "MediaService"
        const val PLAY = 0
        const val STOP = 1
        const val CHANNEL_DEFAULT_IMPORTANCE = "Channel_Yudha"
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (action != null) {
            when (action) {
                ...
                ACTION_DESTROY -> if (mMediaPlayer.isPlaying) {
                    stopSelf()
                } else -> {
                    init()
                }
